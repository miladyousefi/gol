 ...
 */
int (*compte_voisins_vivants) (int, int, grille) = compte_voisins_non_cyclique ; // developed
/** \brief function pointer (updates a living cell):
  * points to set_viellit or to set_ne_viellit_pas (depending on the current rule) 
 */
void (*set_continue_vie)(int,int,grille) = set_ne_viellit_pas ; // developed
...
int compte_voisins_non_cyclique (int i, int j, grille g){ // developed
	int v = 0, l=g.nbl, c = g.nbc;
	...
	for (i=0; i<l; i++)
	{
		for (j=0; j<c; ++j)
		{
			...
			{ // evolution of a living cell 
				...
				else set_continue_vie(i,j,*g); // developed
			}
			else 
